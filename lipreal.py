...
def read_imgs(img_list):
    ...
    try:
        for img_path in tqdm(img_list):
            # Suppress libpng "Read Error" from cv2.imread (libpng writes to C fd 2, not sys.stderr)
            try:
...
class LipReal(BaseReal):
    ...
    def paste_back_frame(self,pred_frame,idx:int):
        ...
        y1, y2, x1, x2 = bbox
        # Target region shape (bbox can be (y1,y2,x1,x2) or (x1,y1,x2,y2); use slice shape so we always match)
        roi = combine_frame[y1:y2, x1:x2]
        ...
        # Ensure exact match (handles bbox/coords inconsistencies across avatar frames)
