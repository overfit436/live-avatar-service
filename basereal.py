...
class BaseReal:
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        while not quit_event.is_set():
            ...
            # Guard: avatar frame_list_cycle can contain None if some images failed to load
            if combine_frame is None or not (hasattr(combine_frame, 'dtype') and hasattr(combine_frame, 'shape')):
                if _last_valid_frame is not None:
                    combine_frame = _last_valid_frame
                else:
                    combine_frame = np.zeros((450, 450, 3), dtype=np.uint8)
            else:
                _last_valid_frame = combine_frame
